...
def training(start_train_date, end_train_date, resume, diff_days):
    ...
    config["inner_adaptation_steps"] = 2 # Number of Inner adaptation steps for the MAML algorithm
    config["maml_optimizer_steps"] = 5 # Number of MAML steps per meta-update iteration (PPO steps)
    ...
    config["lr"] = 1e-3 # Stepsize of SGD
    config["lr"] = tune.grid_search([0.01, 0.001, 0.0001])
    config["explore"] = True
    config["clip_actions"] = False
    #config["observation_filter"] = 'MeanStdFilter'
    config["batch_mode"] = "complete_episodes"
    config["num_workers"] = 3 # num_cores
    config["num_gpus"] = 0
    config["env"] = "simplePible"
    #config["model"] = {
    #    "fcnet_hiddens": [64, 64],
    #    "free_log_std": True,
    ...
    analysis = ray.tune.run(
        ...
        config={
            ...
            "inner_adaptation_steps": tune.grid_search([1, 5, 10]), # Number of Inner adaptation steps for the MAML algorithm
            "maml_optimizer_steps": tune.grid_search([1, 5, 10]),
            "lr": tune.grid_search([1e-3, 1e-4, 1e-5]),
            "inner_lr": tune.grid_search([1e-3, 1e-4, 1e-5]),
            #"eager": False,
            "env_config": {
                "settings": settings,
                "main_path": curr_path,
